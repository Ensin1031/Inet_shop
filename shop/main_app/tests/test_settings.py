...
from shopping.models import CategoryDB, BrandNameDB, GalleryDB, ReviewsDB, \
    GoodsDB
...
class SettingsTestCases(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_image_good_01 = tempfile.NamedTemporaryFile(
            suffix='.jpg').name
        cls.test_image_good_02 = tempfile.NamedTemporaryFile(
            suffix='.jpg').name
        # БД: тестовый юзер
        ...
        cls.test_category_parent = CategoryDB.objects.create(
            title='testCategoryParent'
        )
        cls.test_category = CategoryDB.objects.create(
                title='testCategory',
                parent=cls.test_category_parent
        )
        ...
        cls.test_promo = PromotionDB.objects.create(
                ...
                description='TestPromoDescription TestPromoDescription '
                            'TestPromoDescription',
...
class ForGetFuncTests(SettingsTestCases):
    ...
    def setUp(self) -> None:
        self.test_category_fgf = CategoryDB.objects.create(
            title='testCategoryFGF', parent=self.test_category_parent)
        ...
        self.test_list = ShowObjects(
                GoodsDB.objects.filter(title__contains='test')
        )
