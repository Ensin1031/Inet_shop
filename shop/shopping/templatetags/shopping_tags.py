...
def show_band(good):
    ...
    goods = good.category.category_good.filter(presence=True)\
        .filter(category=good.category).order_by('-n_views')[:8]\
...
def show_sidebar(request):
    ...
    count = {
        ...
        'brands': BrandNameDB.objects.annotate(
                cnt=Count('brand_good', filter=F('brand_good__presence'))
        ),
        'category': CategoryDB.objects.annotate(
                cnt=Count('category_good', filter=F('category_good__presence'))
        ),
        'request': request,
    }
