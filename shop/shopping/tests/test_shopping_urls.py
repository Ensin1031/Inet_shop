...
class ShoppingUrlsTests(SettingsTestCases):
    ...
    def test_category_url_response(self):
        response = self.client.get(reverse('category', args=[self.test_category.slug]))
        self.assertEqual(200, response.status_code)

    def test_category_url_is_resolved(self):
        url = reverse('category', args=[self.test_category.slug])
        self.assertEqual('shopping/category/<str:slug_cat>/', resolve(url).route)
        self.assertEqual(self.test_category.slug, resolve(url).kwargs['slug_cat'])
        self.assertEqual(ShowAllGoods, resolve(url).func.view_class)

    def test_brand_url_response(self):
        response = self.client.get(reverse('brand', args=[self.test_brand.slug]))
        self.assertEqual(200, response.status_code)

    def test_brand_url_is_resolved(self):
        url = reverse('brand', args=[self.test_brand.slug])
        self.assertEqual('shopping/brand/<str:slug_brand>/', resolve(url).route)
        self.assertEqual(self.test_brand.slug, resolve(url).kwargs['slug_brand'])
        self.assertEqual(ShowAllGoods, resolve(url).func.view_class)
